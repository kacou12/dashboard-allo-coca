  ...
  <div class="px-4 w-full">
    ...
    <header class="flex items-center gap-2 text-white">
      <section class="spacep-y-1">
        <h1 class="text-clamp-md font-semibold font-merriweathersans">
          Commandes
        </h1>
        <p class="text-sm text-white">
          <span v-if="isFetched">{{ ordersData?.total }}</span> commandes
          trouvées
        </p>
      </section>
    </header>
    ...
    <section class="flex xl:justify-between items-center">
      <section class="flex items-center gap-2">
        <CommonSelect
          v-model="statusModel"
          :default-width="width >= 1366 ? 'w-fit' : 'w-full'"
          class="w-full"
          title="Filtre par statut"
          :elements="[
            { name: 'Filtre par statut', value: 'all' },
            ...orderStatusConfigEnumData.map((item) => ({
              name: item.name,
              value: item.value,
            })),
          ]"
        ...
        <section class="my-5 xl:min-w-[300px] min-w-[200px] mx-1">
          <SearchBar
            :is-loading="isFetching && filters.search !== undefined"
    ...
    <section class="w-full">
      <CommonDataTable
        :page-size="limitModel ? parseInt(limitModel) : 10"
        ref="my-table"
        :default-page="filters.page"
        :total="ordersData?.total ?? 0"
        :columns="allococaOrdersColumns"
...
const updateData = () => {
  ...
  filters.dates = [
    dates.value.start!.toDate("Africa/Abidjan"),
    dates.value.end!.toDate("Africa/Abidjan"),
  ];
};
