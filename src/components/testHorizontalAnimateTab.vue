...
<template>
    <div class="w-fit bg-gray-50 rounded-lg">
        <div class="relative">
            <!-- Liste des tabs -->
            <div ref="tabsList" class="flex p-1 space-x-1 bg-gray-100 rounded-lg">
                <button v-for="(tab, index) in tabs" :key="tab.id" @click="switchTab(tab.id, index)"
                    class="relative z-20 px-3 py-1.5 text-sm font-medium transition-all duration-200 rounded-md" :class="[
                        activeTab === tab.id
                            ? 'text-gray-900'
