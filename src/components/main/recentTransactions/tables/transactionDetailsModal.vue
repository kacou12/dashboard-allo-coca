<template>
    <CommonModal v-model:open="open" backgroudColor="bg-neutral-80" :showDivider="false">
        ...
        <template #title>
            <h2 class="font-merriweathersans font-bold text-[20px] text-neutral-10">Paiement</h2>
        </template>
        <template #content class=" ">
            <!-- First section  -->

            <section class="flex flex-col gap-2 p-3 rounded-xl bg-white">
                <div class="flex justify-between items-center">
                    <img class="w-[33px] h-[33px]"
                        :src="transaction.payer_provider?.image_url ?? 'http://placehold.jp/150x150.png'" alt="">
                ...
                <div>
                    <p class="text-neutral-40 text-sm">Envoi a {{ transaction.beneficiary_name }} ({{

                        transaction.beneficiary_phone }})</p>
                </div>

            </section>
            <section class="text-sm px-6 space-y-5  p-3 rounded-xl bg-white mt-2">
        ...
        <template #footer>
            <section class="p-3 rounded-b-xl bg-white mt-2"></section>
