  ...
  <div class="">
    <Dialog v-model:open="open">
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="end"
          class="text-xs text-neutral-10 font-worksans"
        >
          ...
          <DialogTrigger as-child v-if="order.status != 'paid'">
            <DropdownMenuItem @click="contentModalAction = 'PAID'"
              >Marquer comme "payé"</DropdownMenuItem
            >
          </DialogTrigger>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      >
        <article v-if="contentModalAction == 'CANCELLED'">
          ...
          <DialogFooter class="flex gap-2 px-6 pb-6">
            <Button
              class="flex-1 py-[22px] text-sm rounded-lg font-medium"
              variant="outline"
              @click="open = false"
            >
              Annuler
            ...
            <Button
              @click="canceledOrderHandler"
              class="flex-1 py-[22px] text-sm rounded-lg bg-primary font-normal"
              variant="default"
            >
              Confirmer
            </Button>
...
import {
  orderStatusConfigEnumData,
  type OrderResponse,
} from "@/services/allococa/orders/order-type";
